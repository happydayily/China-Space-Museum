...
V5.0-F，已完成 F1/F2 两轮数字博物馆展品化、轻交互、来源降噪、桌面与移动端自动验收，以及 `v5.0-f` 版本归档。
...
- 统一 1600×900 视口的六页桌面自动验收截图
- 统一 390×844 视口的六页移动端自动验收截图
...
生成六张桌面和六张移动端最新长页面验收截图：

```bash
npm run screenshots
```

桌面截图输出到 `docs/screenshots/review/latest/`，移动端截图输出到 `docs/screenshots/review/latest/mobile/`；明确的版本验收图另行归档到 `docs/screenshots/<版本号>/`。截图脚本复用本机 Edge/Chrome，不保存浏览器 Profile、缓存或 Cookie，并自动检查图片、链接、错误覆盖层和移动端横向溢出。
...
- V5.0-F 已完成两轮：F1 完成五厅展品化与视觉层级重构，F2 完善影像叙事、轻交互与来源体系；当前等待人工/ChatGPT 对桌面与移动端截图进行最终验收，不创建正式 `v5.0` 标签。
