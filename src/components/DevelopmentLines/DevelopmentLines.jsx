      ...
      <div className="development-network-wrap">
        <svg className="development-network" viewBox="0 0 1010 365" role="img" aria-label="中国航天五大发展主线与关键交汇关系">
          ...
          {lanes.map((lane) => {
            ...
            return <g className={`network-lane ${dimmed ? 'is-dimmed' : ''} ${hovered === lane.id ? 'is-hovered' : ''}`} key={lane.id} onMouseEnter={() => setHovered(lane.id)} onMouseLeave={() => setHovered(null)} onClick={() => onEnter(line.grandHallId)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onEnter(line.grandHallId) }} role="button" tabIndex="0" aria-label={`进入${line.name}主题展厅`}>
