...
export default function SatelliteNetworkStory({ hall, nodes, missions, onOpenMission }) {
  ...
  return (
    <div className="story-page story-page--satellite">
      ...
      <StoryTour items={[{ id: 'story-why', label: '为什么重要' }, { id: 'satellite-core', label: '数据落地' }, { id: 'deep-dive', label: '服务证据' }, { id: 'story-next', label: '下一步' }]} />
      ...
      <section className="grand-story grand-story--satellite" id="satellite-core">
        <div className="story-heading"><span className="section-kicker">卫星应用网络</span><h2>先问三个问题，<br />再看卫星怎样工作。</h2><p>天基系统的价值不在于星座图本身，而在于它持续把观测、定位和通信数据送到地面，变成可以依赖的社会服务。</p></div>
          <div className="satellite-use-cases" aria-label="卫星应用进入日常生活的三个问题与结果">
          <article><span>01 · 导航</span><h3>我在哪里？</h3><strong>北斗三号</strong><p>定位、导航与授时进入交通、农业、测绘和应急系统。</p><div className="satellite-output"><small>地面得到</small><b>可调用的位置与时间</b></div><StoryMedia ids={['official-beidou-3-constellation-2020']} mode="compact" color={hall.color} /></article>
          <article><span>02 · 气象</span><h3>天气正在发生什么？</h3><strong>风云卫星</strong><p>连续观测云图、台风和天气变化，让预报与灾害响应有了共同的天空视野。</p><div className="satellite-output"><small>地面得到</small><b>持续更新的云图与预警依据</b></div><StoryMedia ids={['official-fy4a-cloud-image-2017']} mode="feature" color={hall.color} /></article>
          <article><span>03 · 遥感</span><h3>从太空能看清什么？</h3><strong>高分工程</strong><p>对地观测把土地、海洋和灾害变化转化为可分析的数据。</p><div className="satellite-output"><small>地面得到</small><b>可比对的地表变化影像</b></div><StoryMedia ids={['open-gaofen-3-radar-images-2022']} mode="feature" color={hall.color} /></article>
        </div>
