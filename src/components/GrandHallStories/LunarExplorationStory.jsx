  ...
  return (
    <div className="story-page story-page--lunar">
      ...
      <section className="grand-story grand-story--lunar" id="lunar-core">
        ...
        <div className="lunar-route-map"><div className="lunar-main-route">{stages.map(([stage, title, year, description, mediaId]) => <article key={stage}><b>{stage}</b><span /><time>{year}</time><h3>{title}</h3><p>{description}</p>{mediaId ? <StoryMedia ids={[mediaId]} mode="inline" color={hall.color} /> : null}</article>)}</div><aside className="lunar-side-branch" id="lunar-evidence" aria-label="月背工程支路"><header><span>从“落”分出的月背支路</span><strong>通信 → 软着陆 → 采样返回</strong></header><div className="lunar-branch-sequence"><article><b>通信</b><strong>鹊桥中继</strong><span>先让月背能与地面保持链路</span></article><i aria-hidden="true">↓</i><article><b>软着陆</b><strong>嫦娥四号</strong><span>首次在月球背面软着陆巡视</span></article><i aria-hidden="true">↓</i><article><b>采样返回</b><strong>嫦娥六号</strong><span>首次从月背带回样品</span></article></div><StoryMedia ids={['media-lunar-change6-sample-process']} mode="inline" color={hall.color} /></aside></div>
